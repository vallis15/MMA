...
export const FightVisualizer: React.FC<FightVisualizerProps> = ({ logs, isComplete }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <div className="flex justify-between mb-2">
            ...
            <span className={`text-sm font-bold ${playerHealth > 30 ? 'text-forge-gold' : 'text-red-400'}`}>
              {Math.ceil(playerHealth)} HP
            </span>
          </div>
          <div className="h-6 bg-iron-light rounded border border-iron-mid overflow-hidden">
            <motion.div
              initial={{ width: '100%' }}
              ...
              className={`h-full transition-colors ${
                playerHealth > 50
                  ? 'bg-gradient-to-r from-forge-gold to-green-500'
                  : playerHealth > 25
          ...
          {playerHealth <= 0 && (
            <div className="mt-2 text-center font-bold text-red-400 text-sm">🔥 KNOCKOUT! 🔥</div>
          )}
        </div>
        ...
        <div>
          <div className="flex justify-between mb-2">
            ...
            <span className={`text-sm font-bold ${opponentHealth > 30 ? 'text-forge-gold' : 'text-red-400'}`}>
      ...
      {/* Fight Log */}
      <div className="bg-iron-mid border border-red-800/50 rounded-lg p-6 max-h-96 overflow-y-auto space-y-2">
        ...
        {isComplete && displayedLogs.length > 0 && (
          <motion.div
            ...
            className="pt-4 mt-4 border-t border-red-800/30 text-center"
          >
